...
index70 = min(find(eng_stress > approx_yield_strength*0.6));
index40 = min(find(eng_stress > approx_yield_strength*0.3));
%%%%%%%%%%%%%%%%%%%%%%%%%
...
index_yield = min(find(exploted_stress_full > Yield_strength(1)));
...
max_compression=50;
[minValue, closestIndex] = min(abs(eng_strain - max_compression));
ultimate_tensile_stress = eng_stress(closestIndex);
...
% Legend
Syt = ['Yield Strength at ', num2str(round(Yield_strength(1),0)),' MPa' ];
...
index_yield_strain = min(find(eng_strain > Strain_Yield(1)));
epsilon_0 = Strain_Yield(1)/100;


%% %%%%%%%%%%%%% Plotting true stress Vs True Strain %%%%%%%%%%%%

epsilon =-1*log(1-eng_strain([index_yield_strain:index_ultimate])*0.01);
plastic_strain = smoothdata(epsilon,'sgolay')-epsilon_0;
true_stress = eng_stress([index_yield_strain:index_ultimate]).*(1-eng_strain([index_yield_strain:index_ultimate])*0.01);
figure()
plot(plastic_strain,true_stress)
hold on
%% Plotting VPSC output with true stress-strain data
...
sigma_minus_sigma_0 = sigma-Yield_strength(1);
